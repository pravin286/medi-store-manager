...
router.post("/stores", requireAuth, async (req, res) => {
    ...
    const user = req.user;
    const d = parsed.data;
    const [result] = await db.execute(`INSERT INTO stores 
...
router.get("/stores/my", requireAuth, async (req, res) => {
    const [rows] = await db.execute("SELECT * FROM stores WHERE owner_id=? ORDER BY created_at DESC", [req.user.id]);
    res.json(rows);
});
// ================= GET CITIES =================
router.get("/stores/cities", async (_req, res) => {
    try {
        const [rows] = await db.execute("SELECT DISTINCT city FROM stores");
...
router.patch("/stores/:id", requireAuth, async (req, res) => {
    ...
    await db.execute(`UPDATE stores SET ${fields.join(",")} WHERE id=?`, [...values, id]);
...
router.post("/stores/:id/approve", requireAuth, requireAdmin, async (req, res) => {
    const rawId = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    const id = parseInt(rawId, 10);
    await db.execute("UPDATE stores SET status='approved', rejection_reason=NULL WHERE id=?", [id]);
    res.json({ success: true });
});
// ================= REJECT =================
router.post("/stores/:id/reject", requireAuth, requireAdmin, async (req, res) => {
    ...
    await db.execute("UPDATE stores SET status='rejected', rejection_reason=? WHERE id=?", [reason, id]);
    res.json({ success: true });
});
// ================= ADMIN =================
router.get("/admin/stores", requireAuth, requireAdmin, async (_req, res) => {
    const [rows] = await db.execute("SELECT * FROM stores ORDER BY created_at DESC");
    res.json(rows);
});
