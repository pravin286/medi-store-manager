...
import { useGetStore, useUpdateStore, getAdminListStoresQueryKey, getGetStoreQueryKey, getListStoresQueryKey } from "@workspace/api-client-react";
    ...
    updateMutation.mutate(
      { id: storeId, data },
      {
        onSuccess: () => {
          ...
          toast({
            title: "Store Updated!",
            description: "Store details have been successfully updated.",
          });
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        <Button
          ...
          onClick={() => setLocation(`/admin/stores/${storeId}`)}
        >
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back to Store Details
        </Button>
        <h1 className="text-3xl font-bold tracking-tight text-foreground">Edit Store — Admin</h1>
        <p className="text-muted-foreground mt-1">
          Editing <span className="font-medium text-foreground">{store.storeName}</span>. Changes take effect immediately.
        </p>
      </div>
